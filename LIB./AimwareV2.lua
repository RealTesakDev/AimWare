...
local function plrspeedEnabled()
...
-- Function to apply Chams to all plrs
local function applyChamsToAllplrs()
    if not ESPEnabled then
        return
    end

    -- Apply the Chams effect to all plrs currently in the game
    for _, player in ipairs(plrs:Getplrs()) do
...
-- Event listeners for plrs joining or leaving
...
-- Function to apply ESP to all plrs
local function applyEspToAllplrs()
    if not NAMETAGSEnabled then
        return
    end

    -- Apply the ESP effect to all plrs currently in the game
    for _, player in ipairs(plrs:Getplrs()) do
...
local function GetClosestPlayer()
    ...
    for _, v in ipairs(plrs:Getplrs()) do
    ...
    spamsettings = {
        ...
        real = {
            ...
            [2] = {
                ...
                "get Aimware",
                "buy Aimware",
                "Aimware is no longer a paste",
                "skidhub never, Aimware forever",
...
if ({ identifyexecutor() })[1] == "Krampus" then
    cheat.fonts.plex = Drawing.new("Font", "Aimware.plex")
    cheat.fonts.mono = Drawing.new("Font", "Aimware.mono")
    cheat.fonts.spixel7 = Drawing.new("Font", "Aimware.spixel7")
    if not isfolder("Aimware") then
        makefolder("Aimware")
    end
    if not isfolder("Aimware/assets") then
        makefolder("Aimware/assets")
    end
    if not isfile("Aimware/assets/plex.ttf") then
        local font
        ...
        writefile("Aimware/assets/plex.ttf", font)
    end
    if not isfile("Aimware/assets/mono.ttf") then
        local font
        local success, error = pcall(function()
            font = game:HttpGet("http://31.210.171.229:3000/assets/mono.ttf")
        end)
        if error then print("error while downloading font: " .. error) end
        writefile("Aimware/assets/mono.ttf", font)
    end
    if not isfile("Aimware/assets/spixel7.ttf") then
        local font
        local success, error = pcall(function()
            font = game:HttpGet("http://31.210.171.229:3000/assets/spixel7.ttf")
        end)
        if error then print("error while downloading font: " .. error) end
        writefile("Aimware/assets/spixel7.ttf", font)
    end
    cheat.fonts["plex"].Data = readfile("Aimware/assets/plex.ttf")
    cheat.fonts["mono"].Data = readfile("Aimware/assets/mono.ttf")
    cheat.fonts["spixel7"].Data = readfile("Aimware/assets/spixel7.ttf")
end
print('load_' .. tostring(counter))
counter = counter + 1
EnemyEspTab:AddDropdown('Aimware font',
    {
...
do
    do
        ...
        --[[local spam_words = {
            ...
            "Aimware", "Server Backdoor", "Serverside", "2021", "Working", "(WORKING)", "瞄准无声目标绕过", "Gamesense", "Onetap", "PD Exploit", "Project Delt",
            ...
            "🥳🥳🥳", "RELEASE", "IP RESOLVER", "Infinite Wall Bang", "Wall Bang", "Trickshot", "Sniper", "Wall Hack", "😍😍", "🤩", "🤑", "😱😱", "Free Download Aimware", "Taps", "Owns",
            "Owns All", "Trolling", "Troll", "Grief", "Kill", "弗吉艾尺艾杰开", "Aimware", "kavkaznation", "JSON", "Aimware Developers",
        ...
        --[[local spam_original = {
            ...
            "how does it feel to not have Aimware 🤣🤣🤣😂😂😹😹😹 ",
            "im a firing my laza! 🙀🙀🙀 ",
            "😂😂😂😂😂GAMING CHAIR😂😂😂😂😂",
            "retardheadass",
            ...
            'table.find(charts, "any other script other than Aimware") -> nil 💵💵💵',
            ...
            "y",
            "b",
            "before Aimware 😭 📢                after Aimware 😁😁😜                    don't be like the person who doesn't have Aimware",
            "                            MADE YOU LOOK ",
            ...
            "god i wish i had Aimware..... 🙏🙏🥺🥺🥺                                                    plzzzzz brooooo 🛐 GIVE IT🛐🛐",
            "swum huh                                                 ",
            "votekick him!!!!!!! 😠 vk VK VK VK VOTEKICK HIM!!!!!!!!! 😠 😢 VOTE KICK !!!!! PRESS Y WHY DIDNT U PRESS Y LOL!!!!!! 😭 ", -- shufy made this
            "Aimware omg omggg omggg its Aimware its Aimware OMGGG!!!  🙏🙏🥺🥺😌😒😡",
            "HOw do you get ACCESS to this Aimware ", -- end
            "I NEED ACCESS 🔑🔓 TO Aimware 🤖📃📃📃 👈 THIS THING CALLED Aimware SCRIPT, I NEED IT ",
